(function ($) {
    ...
    $(document).on('change', '.wpcbm_time select:not(.wpcbm_time_type), .wpcbm_time input:not(.wpcbm_time_val)', function () {
        var val = $(this).val();
        var show = $(this).closest('.wpcbm_time').find('.wpcbm_time_type').find(':selected').data('show');

        $(this).closest('.wpcbm_time').find('.wpcbm_time_val').data(show, val).val(val).trigger('change');
    });

    $(document).on('click touch', '.wpcbm-activate-btn', function (e) {
        e.preventDefault();

        let $this = $(this), id = $this.data('id'), act = $this.hasClass('deactivate') ? 'deactivate' : 'activate';
    ...
    $('#wpcbm_add_image').on('click touch', function () {
        ...
        wpcbm_media.on('select', function () {
            ...
            $('#wpcbm_preview .wpcbm-badge .wpcbm-badge-inner').html('<img src="' + attachment.url + '"/>');
        });
    ...
    $('#wpcbm_styles input[type=radio]').on('change', function () {
        ...
        if (this.value === 'image') {
            $badge.find('.wpcbm-badge-inner').html('<img src="' + $('#wpcbm_image_img').attr('src') + '"/>');
        } else {
    ...
    $('#wpcbm_apply').on('change', function () {
        ...
        $('#wpcbm_configuration_new').hide();

        if (apply === '' || apply === 'none' || apply === 'all' || apply === 'sale' || apply === 'featured' || apply === 'bestselling' || apply === 'instock' || apply === 'outofstock' || apply === 'backorder') {
            return;
        }

        if (apply === 'new') {
            $('#wpcbm_configuration_new').show();
            return;
        }
    ...
    $(document).on('click touch', '#wpcbm_icons_btn', function (e) {
        ...
        $('#wpcbm_dialog_icons').dialog({
            minWidth: 460, modal: true, dialogClass: 'wpc-dialog', open: function () {
    ...
    $(document).on('click touch', '#wpcbm_shortcodes_btn', function (e) {
        ...
        $('#wpcbm_dialog_shortcodes').dialog({
            minWidth: 460, modal: true, dialogClass: 'wpc-dialog', open: function () {
                $('.ui-widget-overlay').bind('click', function () {
    ...
    $(document).on('change', '.wpcbm_conditional_value', function () {
        var apply = $(this).closest('.wpcbm_conditional').find('.wpcbm_conditional_apply').val();
        ...
        $('#wpcbm_terms').selectWoo({
            ajax: {
                url: ajaxurl, dataType: 'json', delay: 250, data: function (params) {
                    return {
                        q: params.term, action: 'wpcbm_search_term', nonce: wpcbm_vars.nonce, taxonomy: apply,
                    };
    ...
    function wpcbm_badges_select() {
        $('#wpcbm_badges').selectWoo({
            ajax: {
                url: ajaxurl, dataType: 'json', delay: 250, data: function (params) {
                    return {
                        action: 'wpcbm_search_badges', nonce: wpcbm_vars.nonce, q: params.term,
                    };
                }, processResults: function (data) {
                    var options = [];
                    if (data) {
    ...
    function wpcbm_timer_init($time) {
        if (typeof $time !== 'undefined') {
            ...
            $time.find('.wpcbm_show_if_' + show).show();
        } else {
            $('.wpcbm_time').each(function () {
                var show = $(this).find('.wpcbm_time_type').find(':selected').data('show');
    ...
    function wpcbm_timer_picker() {
        $('.wpcbm_dpk_date_time:not(.wpcbm_dpk_init)').wpcdpk({
            timepicker: true, onSelect: function (fd, d, dpk) {
                ...
                var show = dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_type').find(':selected').data('show');

                dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_val').data(show, fd).val(fd).trigger('change');
            },
        ...
        $('.wpcbm_dpk_date_range:not(.wpcbm_dpk_init)').wpcdpk({
            range: true, multipleDatesSeparator: ' - ', onSelect: function (fd, d, dpk) {
                if (!d) {
                    return;
                }
        ...
        $('.wpcbm_dpk_date_multi:not(.wpcbm_dpk_init)').wpcdpk({
            multipleDates: 5, multipleDatesSeparator: ', ', onSelect: function (fd, d, dpk) {
                if (!d) {
                    return;
                }
        ...
        $('.wpcbm_dpk_time:not(.wpcbm_dpk_init)').wpcdpk({
            timepicker: true, onlyTimepicker: true, classes: 'only-time', onSelect: function (fd, d, dpk) {
                if (!d) {
                    return;
                }
                ...
                if (dpk.$el.hasClass('wpcbm_time_from') || dpk.$el.hasClass('wpcbm_time_to')) {
                    var time_range = dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_from').val() + ' - ' + dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_to').val();

                    dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_val').data(show, time_range).val(time_range).trigger('change');
                } else {
                    dpk.$el.closest('.wpcbm_time').find('.wpcbm_time_val').data(show, fd).val(fd).trigger('change');
                }
            },
    ...
    function wpcbm_conditional_init() {
        $('.wpcbm_conditional_apply').each(function () {
            var $this = $(this);
            var $value = $this.closest('.wpcbm_conditional').find('.wpcbm_conditional_value');
            var $select_wrap = $this.closest('.wpcbm_conditional').find('.wpcbm_conditional_select_wrap');
            var $select = $this.closest('.wpcbm_conditional').find('.wpcbm_conditional_select');
            var $compare = $this.closest('.wpcbm_conditional').find('.wpcbm_conditional_compare');
            var apply = $this.val();
            var compare = $compare.val();

            if (apply === 'sale' || apply === 'featured' || apply === 'bestselling' || apply === 'instock' || apply === 'outofstock' || apply === 'backorder') {
                $compare.hide();
                $value.hide();
                $select_wrap.hide();
            } else {
                ...
                if (apply === 'price' || apply === 'rating' || apply === 'release' || apply === 'stock') {
                    $select_wrap.hide();
                    $value.show();
                    $compare.find('.wpcbm_conditional_compare_price option').prop('disabled', false);
                    $compare.find('.wpcbm_conditional_compare_terms option').prop('disabled', true);
                    ...
                    $select_wrap.show();
                    $value.hide();
                    $compare.find('.wpcbm_conditional_compare_price option').prop('disabled', true);
                    $compare.find('.wpcbm_conditional_compare_terms option').prop('disabled', false);
            ...
            if ((typeof $select.data(apply) === 'string' || $select.data(apply) instanceof String) && $select.data(apply) !== '') {
        ...
        $('.wpcbm_conditional_select').each(function () {
            ...
            $this.selectWoo({
                ajax: {
                    url: ajaxurl, dataType: 'json', delay: 250, data: function (params) {
                        return {
                            action: 'wpcbm_search_term', nonce: wpcbm_vars.nonce, q: params.term, taxonomy: apply,
                        };
                    }, processResults: function (data) {
                        var options = [];
                        if (data) {
    ...
    function wpcbm_toggle_type() {
        $('input[name="wpcbm_type"]:checked').each(function () {
            ...
            if (val === 'overwrite' || val === 'prepend' || val === 'append') {
                $(this).closest('.wpcbm_table').find('.wpcbm_show_if_overwrite').show();
            } else {
                $(this).closest('.wpcbm_table').find('.wpcbm_show_if_overwrite').hide();
            }
        });
    }
